"""Dask backend that accepts inputs as dask.array.Array or numpy.ndarray

output are always returned as numpy.ndarray
"""
...
def __ua_convert__(value, dispatch_type, coerce):
    ...
    if dispatch_type is np.ndarray:
        if not coerce and not isinstance(value, (np.ndarray, da.Array)):
    ...
    if dispatch_type is scalar_or_array:
        if np.isscalar(value):
            return value
        elif not coerce and not isinstance(value, (np.ndarray, da.Array)):
            return NotImplemented
        return asdask(value)
...
def __ua_function__(method, args, kwargs):
    ...
    if isinstance(out, da.Array):
        numpy_out = np.asarray(out)
    else:
        numpy_out = []
        for o in out:
            if isinstance(o, da.Array):
                numpy_out.append(np.asarray(o))
            else:
                numpy_out.append(o)
    return numpy_out
