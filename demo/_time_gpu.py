...
class _PerfCaseResult:
    ...
    @staticmethod
    def _to_str_per_item(device_name, t, unit='ms'):
        assert t.ndim == 1
        assert t.size > 0
        ...
        if unit == 'ns':
            ...
            raise ValueError(
                "unit must be one of {'auto', 'ns', 'us', 'ms', 's'}"
            )

        s = f'    {device_name}:{t.mean():9.03f} {unit}'
        if t.size > 1:
            s += f'   +/-{t.std():6.03f}'
            s += f' (min:{t.min():9.03f} / max:{t.max():9.03f}) {unit}'
        return s

    def to_str(self, show_cpu=False, show_gpu=True):
        results = []
        if show_cpu:
            results.append(self._to_str_per_item('CPU', self._ts[0]))
        if show_gpu:
            for i, d in enumerate(self._devices):
                results.append(
                    self._to_str_per_item('GPU-{}'.format(d),
                                          self._ts[1 + i]))
        return '{:<25s}:{}'.format(self.name, ' '.join(results))

    def __str__(self):
        return self.to_str(show_cpu=False, show_gpu=True)
...
def _repeat_dask(
    ...
    for i in range(n_warmup):
        ...
        if hasattr(out, 'compute'):
            out.compute()
        elif isinstance(out, (tuple, list)) and all(hasattr(o, 'compute') for o in out):
            [o.compute() for o in out]
        else:
            raise ValueError("unsupported dask output type")
